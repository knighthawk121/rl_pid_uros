...
/*/ Send feedback
void sendFeedback(int currentPos, int error) {
  if (!motor.goalActive) return;

  current_feedback.current_position = currentPos;
  current_feedback.target_position = motor.target;
  current_feedback.current_error = error;

  rcl_ret_t ret = rclc_action_publish_feedback(&current_goal_handle, &current_feedback);
  if (ret != RCL_RET_OK) {
    Serial.printf("Feedback error: %d\n", ret);
  }
}*/
